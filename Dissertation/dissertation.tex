...
Using the Pearson's Correlation Coefficient (Equation \ref{eq:pearson}), a correlation matrix was derived from the binary purchase vector dataset, and is illustrated in Figure \ref{fig:correlation}. In this instance - because the correlation is of binary variables - the Pearson's Correlation Coefficient is equivalent to the Phi Coefficient ($\phi$) \cite{phi}. The resulting correlation matrix is also diagonally symmetrical due to its nature (i.e. $\phi_{ij} = \phi_{ji}$).
\begin{equation}
\label{eq:pearson}
