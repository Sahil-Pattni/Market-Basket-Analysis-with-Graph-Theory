...
\caption{Pruning Infrequent Rules (Source: \cite{apriori-prune})}
\label{fig:prune}
\end{figure}
...
\begin{figure}[H]
\centering
\includegraphics[scale=0.25]{cluster_named}
...
If we were to use permutations, we would have to generate each itemset $i$ such that $(\{i \in A\} \rightarrow \{i \in B\})$ for clusters $A$ and $B$, and then later $(\{i \in B\} \rightarrow \{i \in B\})$, which would essentially be the same combinations in a different configuration. Instead, by using \texttt{combinations} on line $4$ of Listing \ref{lst:bi_cluster}, we only compute the former, and then re-add the rule to the ruleset with the antecedent and consequent swapped (line $12$). This method essentially reduces our computational time by half.
\subsection{Intra-Cluster Rule Generation}
Intra-cluster rules are those where the both antecedent and consequent originate from the same cluster.
